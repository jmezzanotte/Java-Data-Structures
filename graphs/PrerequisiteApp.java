...
/**
 * Courses are vertices edges are used to represent the prerequisite information
...
public class PrerequisiteApp {
    ...
    private static final String COURSE_1_ENTRY = "Enter course 1: ";
    private static final String COURSE_2_ENTRY = "Enter course 2: ";
        ...
        switch(input){
            ...
            case "p" : // Add new prerequisite relationship  
                ...
                if(coursesValid(course1, course2)){
                    ...
                    courses.addEdge(getCourseIndex(course1),
                            getCourseIndex(course2));
                }else{
                    System.out.println("Courses are the same or one course " +
            ...
            case "r" : // remove prerequisite path  
                ...
                if(coursesValid(pathSource, targetSource)){
                    ...
                    courses.removeEdge(getCourseIndex(pathSource),
                            getCourseIndex(targetSource));
                }else{
                    System.out.println("Courses are the same or one course " +
                            "does not exist in the catalog.");
                }
            ...
            case "pp" : // Is there a prerequisite path
                ...
                if(coursesValid(temp1, temp2)){
                    ...
                    if(courses.isPath(getCourseIndex(temp1), getCourseIndex(temp2))){
                        System.out.println("----------------------------");
            ...
            case "i" : // Is course immediate prerequisite
                ...
                String source = validateInput(COURSE_1_ENTRY).trim().toLowerCase();
                String target = validateInput(COURSE_2_ENTRY).trim().toLowerCase();
                ...
                if(coursesValid(source, target)){
                   ...
                   /*
                    try{
                        int[] neighbors = courses.neighbors(getCourseIndex(source));
                        System.out.println(Arrays.toString(neighbors));
                        for(int i = 0; i < neighbors.length; i++){
                            int targetIndex = getCourseIndex(target);
                            if( neighbors[i] == targetIndex){
                                System.out.printf(HEADER, "IMMEDIATE PREREQUISITE");
                                System.out.println(courses.getLabel(targetIndex));
                            }
                        }
                    }catch(NullPointerException e){
                        System.out.println("No Immediate prerequisite");
                    }
                    */
                }
